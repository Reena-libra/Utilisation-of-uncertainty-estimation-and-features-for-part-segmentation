...
#
results_dir=os.path.join(ROOT_DIR,'P2RE_base')
print(results_dir)
...
def main(args):
    ...
    experiment_dir = 'log/part_seg_p2re/' + args.log_dir
    ...
    with torch.no_grad():
        ...
        for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader),
            ...
            points, label, target = points.float().cuda(), label.long().cuda(), target.long().cuda()
                ...
                ####for saliency
            points_new=points          
            print('target',target.shape,points_new.shape)
...
########################################################################################
            ...
            for i in range(cur_batch_size):
                ...
                #seg plotting
                color=['g','r','c','m']
                #color = ['g', 'b', 'c', 'm']  # Colors for each ground truth category
                ground_truth_colors = ['green', 'red']  # [leaves, ear] for ground truth
                prediction_colors = ['green', 'red', 'm']  # [correct leaf, correct ear, misclassified]
                correct_color = 'g'  # Green for correct predictions
                incorrect_color = 'r'  # Red for misclassifications
                fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(10, 12))

                for j in range(num_point):

                    cd=None
                    x=points_new[i:i+1,j,0].cpu().data.numpy()
                    y=points_new[i: i+1,j,1].cpu().data.numpy()
                    z=points_new[i: i+1,j,2].cpu().data.numpy()
                    
                    seg = int(segp[j])  # Prediction
                    ori_seg = int(segl[j])  # Ground truth
                    # Ground truth plot
                    ax[0].scatter(x, y, c=ground_truth_colors[ori_seg], s=3)
                    # Prediction plot
                    #ax[1].scatter(x, y, c=correct_color if seg == ori_seg else incorrect_color, s=3)
        
                    # Misclassification plot (highlight only misclassified points in red)
                    if seg == ori_seg:  # Correct prediction
                       ax[1].scatter(x, y, c=prediction_colors[seg], s=3)
                    else:  # Misclassification
                       ax[1].scatter(x, y, c=prediction_colors[2], s=3)  #
    
                    # Set titles for each subplot
                    #ax[0].set_title('Ground Truth')
                    #ax[1].set_title('Prediction (Green = Correct, Red = Incorrect)')
                    #ax[2].set_title('Misclassifications (Red = Incorrect)')
                    ax[0].set_title('Ground Truth ')
                    ax[1].set_title('Prediction')
               # plt.savefig('result' + str(j) + '.png')
               # plt.savefig(results_dir,f'result_plot_{i}.png')
                plt.tight_layout()
                plt.savefig(os.path.join(results_dir, f'result_plot_{batch_id}.png'))
                plt.clf()
                print(f'Plot saved for sample {i}')
  ...
  ##########################################################################################
    ...
    log_string('Cat 0 avg mIOU is: %.5f' % test_metrics['class_cat1_avg_iou'])
