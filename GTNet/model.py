...
class GTNet_partseg(nn.Module):
    def __init__(self, args, seg_num_all):
        ...
        self.conv6 = nn.Sequential(nn.Conv1d(96*4+3, args.emb_dims, kernel_size=1, bias=False),
                                   self.bn6,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv7 = nn.Sequential(nn.Conv1d(1, 64, kernel_size=1, bias=False),
                                   self.bn7,
                                   nn.LeakyReLU(negative_slope=0.2)) ### chnaged 16 to 1 very first parametere
