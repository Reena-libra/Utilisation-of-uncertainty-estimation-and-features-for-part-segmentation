...
from util import cal_loss, IOStream ,cal_loss_new, mc_dropout_variance,calculate_density
...
def calculate_hybrid_scores(uncertainty, ear_counts, ear_ratios, alpha=1.25, beta=0.75, gamma=0.25):
    """
...
def train(args, io):
   ...
   # test_loader = DataLoader(ShapeNetPart(partition='test', num_points=args.num_points, class_choice=args.class_choice),                             num_workers=12, batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    TRAIN_DATASET=My_H5Dataset(os.path.join(BASE_DIR2, 'wheat_a2re2k/TRAIN_A2RE2K_Wheat.h5'),normal_channel=False)
    VAL_DATASET=My_H5Dataset(os.path.join(BASE_DIR2, 'wheat_a2re2k/VAL_A2RE2K_Wheat.h5'),normal_channel=False)
    ...
    criterion4= calculate_density().to(device)
    ...
    for epoch in range(start_epoch+1,args.epochs+1):
        ...
        for i,(data, label,seg,ears )in enumerate(tqdm(train_loader, total=len(train_loader))):
            ...
            hybrid_scores = calculate_hybrid_scores(uncertainty, feature_importance_scores,ear_ratio)
...
def test(args, io):
    ...
    TEST_DATASET=My_H5Dataset(os.path.join(BASE_DIR2, 'wheat_a2re2k/TEST_A2RE2K_Wheat.h5'),normal_channel=False)
    ...
    model = nn.DataParallel(model)
    #model = model.to(device)
    checkpoint = torch.load('outputs/part_segpart_seg_a2re_m5_extended/models/best_model.pth')
