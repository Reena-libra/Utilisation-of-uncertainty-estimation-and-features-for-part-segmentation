...
def get_graph_feature(x, k=20, idx=None, dim9=False,dim6=False):
    batch_size = x.size(0)
    num_points = x.size(2) ##### we have changed it
    #print('x',x.shape)
...
class GT(nn.Module):
    ...
    def forward(self,features,dim9=False,dim6=False):
        ...
        #print('features and knn features before permutation',features.shape,knn_features.shape)
        features=features.permute(0,2,1)
         
        knn_features=knn_features.permute(0,2,3,1)
        #print('features and knn features after permutation',features.shape,knn_features.shape)
        #batch_size,_,num_points=features.shape ### WE CHANGED IT
        batch_size,num_points,_=features.shape
        #print('batch and num_points',batch_size,num_points)
        #print(self.fb)
        knn_features=self.fb(knn_features)
        #print('knn',knn_features.shape)
        x = self.fc1(features)
        #print('x after knn',x.shape)
        #x = x.permute(0, 2, 1)
        #print('x after knn',x.shape)
        #
