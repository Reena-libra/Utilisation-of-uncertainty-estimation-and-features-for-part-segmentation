...
from util import cal_loss, IOStream ,cal_loss_new, mc_dropout_variance
...
def calculate_hybrid_scores(uncertainty, feature_importance_scores, alpha=1.25, beta=0.75):
    default_weight = 0.05

    # Step 1: Normalize uncertainty
    max_uncertainty = uncertainty.max()
    #print('normalizeeu',uncertainty  / max_uncertainty)
    normalized_uncertainty = uncertainty  / max_uncertainty if max_uncertainty > 0 else uncertainty
    #print('normalized_uncertsa',normalized_uncertainty)
    # Step 2: Assign default weights for zero values in feature importance scores
    normalized_feature_importance = torch.where(
        feature_importance_scores == 0, 
        torch.tensor(default_weight, device=feature_importance_scores.device),
        feature_importance_scores
    )

    # Step 3: Normalize the modified feature importance scores (ear count)
    max_feature_importance = normalized_feature_importance.max()
    if max_feature_importance > 0:
        normalized_feature_importance = (normalized_feature_importance / max_feature_importance)
    else:
        normalized_feature_importance = normalized_feature_importance  # if all are zero, they remain at default_weight
    #print('normalized_feature',normalized_feature_importance)
   # print(alpha * normalized_uncertainty)
   # print(beta * normalized_feature_importance)
    # Step 4: Calculate hybrid scores with balanced alpha and beta
    hybrid_scores = alpha * normalized_uncertainty + beta * normalized_feature_importance
    #hybrid_scores1 =  (normalized_uncertainty +  normalized_feature_importance)
    hybrid_scores = hybrid_scores.mean(dim=1)
    #hybrid_scores1 = hybrid_scores1.mean(dim=1)
    #print(hybrid_scores,hybrid_scores1)
...
def visualization(visu, visu_format, data, pred, seg, label, partseg_colors, class_choice):
    ...
    for i in range(0, data.shape[0]):
        ...
        if skip:
            ...
            IoU = calculate_shape_IoU(np.array(pred_np), np.array(seg_np), label[i].cpu().numpy(), class_choice, visual=False)
...
def train(args, io):
   ...
   # test_loader = DataLoader(ShapeNetPart(partition='test', num_points=args.num_points, class_choice=args.class_choice),                             num_workers=12, batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    TRAIN_DATASET=My_H5Dataset(os.path.join(BASE_DIR2, 'wheat_p2re2k/TRAIN_P2RE2K_Wheat.h5'),normal_channel=False)
    VAL_DATASET=My_H5Dataset(os.path.join(BASE_DIR2, 'wheat_p2re2k/VAL_P2RE2K_Wheat.h5'),normal_channel=False)
    ...
    val_loader = torch.utils.data.DataLoader(VAL_DATASET, batch_size=args.batch_size, shuffle=False, num_workers=10)
    ...
    criterion = cal_loss
    criterion2 = cal_loss_new
    criterion3 = mc_dropout_variance
    ...
    for epoch in range(start_epoch+1,args.epochs+1):
        ...
        for i,(data, label,seg,ears )in enumerate(tqdm(train_loader, total=len(train_loader))):
            seg = seg - seg_start_index
            label_one_hot = np.zeros((label.shape[0], 1))#label.shape[0]=B
            for idx in range(label.shape[0]):
                #label_one_hot[idx, label[idx]] = 1
                label_one_hot[idx,0]=1
            label_one_hot = torch.from_numpy(label_one_hot.astype(np.float32))
            data, label_one_hot, seg, ears = data.to(device), label_one_hot.to(device), seg.to(device), ears.to(device)
            ...
            uncertainty = criterion3(model,data,label_one_hot, num_samples=5)
            #print(uncertainty,ears)
            feature_importance_scores = ears
            
            hybrid_scores = calculate_hybrid_scores(uncertainty, feature_importance_scores)
            seg_pred = seg_pred.permute(0, 2, 1).contiguous()#[B,N,C]
            #loss = criterion(seg_pred.view(-1, seg_num_all), seg.view(-1,1).squeeze())
            loss = criterion2(seg_pred.view(-1, seg_num_all), seg.view(-1,1).squeeze(),hybrid_scores,batch_size=args.batch_size,num_points=data.shape[2])
            # print(loss)
            loss.backward()
            # print(seg_pred.data)
            opt.step()
            pred = seg_pred.max(dim=2)[1]               # (batch_size, num_points)
            intersection, union, target = intersectionAndUnionGPU(pred, seg, 2,255)
        ...
        outstr = 'Train %d, loss: %.6f, train acc: %.6f, train avg acc: %.6f, train iou: %.6f , train_iou0: %.6f,train _iou1:%.6f' % (epoch,                                                                                          train_loss*1.0/count, train_acc,                                                                          avg_per_class_acc,                                                                                                np.mean(train_ious),np.mean(train_ious0),np.mean(train_ious1))
        io.cprint(outstr)
        writer.add_scalar('loss_train', train_loss*1.0/count, epoch)
        writer.add_scalar('mIoU_train', np.mean(train_ious), epoch)
        writer.add_scalar('mIoU_train_0', np.mean(train_ious0), epoch)
        writer.add_scalar('mIoU_train_1', np.mean(train_ious1), epoch)
        ...
        test_label_seg = []
        with torch.no_grad():
            for data,label,seg,ears in tqdm(val_loader, total=len(val_loader)):
                seg = seg - seg_start_index
        ...
        writer.add_scalar('mIoU_val_0', np.mean(test_ious0), epoch)
        writer.add_scalar('mIoU_val_1', np.mean(test_ious1), epoch)
...
def test(args, io):
    ...
    #Try to load models
    #seg_num_all = testDataLoader.dataset.seg_num_all
    #seg_start_index = testDataLoader.dataset.seg_start_index
    #partseg_colors = testDataLoader.dataset.partseg_colors
    seg_num_all =2
    seg_start_index = 0
    if args.model == 'GTNet':
        model = GTNet_partseg(args, seg_num_all).to(device)
    ...
    model = nn.DataParallel(model)
    #model = model.to(device)
    checkpoint = torch.load('outputs/part_seg_p2re_m2/models/best_model.pth')
    ...
    with torch.no_grad():
        for points,label,target,ears in tqdm(testDataLoader, total=len(testDataLoader)):
  ...
  ##########################################################################################
    outstr = 'Test :: test acc: %.6f, test avg acc: %.6f, test iou: %.6f,test classs iou:%.6f, test class iou0:%.6f, test class iou1:%.6f' % (test_metrics['accuracy'],
                                                                             test_metrics['class_avg_accuracy'],
                                                                             test_metrics['inctance_avg_iou'],
                                                                             test_metrics['class_avg_iou'],test_metrics['class_cat0_avg_iou'],test_metrics['class_cat1_avg_iou'])
    io.cprint(outstr)
