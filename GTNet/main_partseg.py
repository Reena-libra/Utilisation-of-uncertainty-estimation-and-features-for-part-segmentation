...
from model_deform import GTNet_partseg
...
def train(args, io):
   ...
   # test_loader = DataLoader(ShapeNetPart(partition='test', num_points=args.num_points, class_choice=args.class_choice),                             num_workers=12, batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    TRAIN_DATASET=My_H5Dataset(os.path.join(BASE_DIR2, 'wheat_g2r/TRAIN_G2R_Wheat.h5'),normal_channel=False)
    VAL_DATASET=My_H5Dataset(os.path.join(BASE_DIR2, 'wheat_g2r/VAL_G2R_Wheat.h5'),normal_channel=False)
    ...
    for epoch in range(start_epoch+1,args.epochs+1):
        ...
        test_label_seg = []
        with torch.no_grad():
            for data,label,seg in tqdm(val_loader, total=len(val_loader)):
                seg = seg - seg_start_index
        ...
        torch.save(latest_state, 'outputs/%s/models/best_model.pth' % args.exp_name)

def test(args, io):
    #test_loader = DataLoader(ShapeNetPart(partition='test', num_points=args.num_points, class_choice=args.class_choice),
      #                       batch_size=args.test_batch_size, shuffle=False, drop_last=False)
    TEST_DATASET=My_H5Dataset(os.path.join(BASE_DIR2, 'wheat_g2r/TEST_G2R_Wheat.h5'),normal_channel=False)
    ...
    model = nn.DataParallel(model)
    #model = model.to(device)
    checkpoint = torch.load('outputs/partseg_g2r_w/models/best_model.pth')
